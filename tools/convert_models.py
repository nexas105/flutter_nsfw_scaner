...
Usage:
    ...
    python tools/convert_models.py --all

    # Or single model:
    python tools/convert_models.py --model adamcodd
"""
...
class _LogitsOnly:
    pass  # placeholder so torch import stays lazy below


def _trace_model(spec: ModelSpec):
    """Load HF checkpoint and trace to TorchScript with logits-only output."""
    import torch
    ...
    # Read class labels from the model's own config — survives label drift.
    ...
    print(f"[{spec.output_name}] Tracing (input {size}x{size})")
    traced = torch.jit.trace(LogitsOnly(model), example, strict=False)
    return traced, class_labels
...
def convert_to_coreml(spec: ModelSpec) -> Path:
    ...
    traced, class_labels = _trace_model(spec)
    size = spec.input_size
...
def compile_and_zip(mlpackage: Path) -> Path:
    """Compile .mlpackage → .mlmodelc and zip it for distribution."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        "--tflite",
        action="store_true",
        help="(Not yet implemented) Also produce TFLite output for Android.",
    )
    args = p.parse_args(argv)

    if args.tflite:
        print(
            "WARN: --tflite not yet implemented. Android plugin currently uses bundled "
            "OpenNSFW2 only — wire up TFLite engine + registry first (see roadmap).",
            file=sys.stderr,
        )
    ...
    if sys.platform != "darwin":
        print(
            "WARN: xcrun coremlc is macOS-only — CoreML compile step will fail on this OS.",
            file=sys.stderr,
        )

    for spec in selected:
        mlpackage = convert_to_coreml(spec)
        compile_and_zip(mlpackage)
    ...
    print()
    print("Filenames must match the URLs in ios/Classes/ml/ModelRegistry.swift exactly.")
    return 0
